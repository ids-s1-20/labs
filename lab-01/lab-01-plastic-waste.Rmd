...
Go to the course [GitHub organization](https://github.com/ids-s1-20) and locate your Lab 01 repo, which should be named `lab-01-plastic-waste-YOUR_GITHUB_USERNAME`. If you're in the right place, it should look like the following.

```{r echo=FALSE, eval=TRUE}
knitr::include_graphics("img/repo-begin.png")
```

Grab the URL of the repo, and clone it in RStudio. Refer to [HW 00](https://ids-s1-20.github.io/homework/hw-00/hw-00-pet-names.html) if you would like to see step-by-step instructions for cloning a repo into an RStudio project.

First, open the R Markdown document `lab-01.Rmd` and Knit it. Make sure it compiles without errors. The output will be in the file markdown `.md` file with the same name.
...
**Warm up 1:** Recall that RStudio is divided into four panes. Without looking, can you name them all and briefly describe their purpose?

**Warm up 2:** Verify that the dataset has loaded into the Environment. How many observations are in the dataset? Clicking on the dataset in the Environment will allow you to inspect it more carefully. Alternatively, you can type `View(plastic_waste)` into the Console to do this. 
...
<i>
**Warm up 1:** Editor; Console; Environment, history, git; Files, plots, packages, help, viewer  
**Warm up 2:** 240  
**Warm up 3:** Not Available, indicates missing values
</i>
</details>
...
1. Convert your side-by-side box plots from the previous task to [violin plots](http://ggplot2.tidyverse.org/reference/geom_violin.html). 
...
1. Visualize the relationship between plastic waste per capita and total population as well as plastic waste per capita and coastal population. You will need to make two separate plots. Do either of these pairs of variables appear to be more strongly linearly associated?
